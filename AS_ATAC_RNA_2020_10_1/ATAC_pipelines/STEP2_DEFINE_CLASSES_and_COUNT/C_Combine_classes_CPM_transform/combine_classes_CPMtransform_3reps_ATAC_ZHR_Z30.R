...
start <- read.delim("./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/BED_files_for_analyses/ZHR_Z30_ATAC_txStart500_counts.bed", header = F) %>% unique()
start$V4 <- NULL
start$class <- "start"
end <- read.delim("./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/BED_files_for_analyses/ZHR_Z30_ATAC_txEnd500_counts.bed", header = F) %>% unique()
end$V4 <- NULL
end$class <- "end"
inter <- read.delim("./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/BED_files_for_analyses/ZHR_Z30_ATAC_intergenic_peak_counts_center1000.bed", header = F) %>% unique()
inter$class <- "inter"
intra <- read.delim("./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/BED_files_for_analyses/ZHR_Z30_ATAC_intragenic_peak_counts_center1000.bed", header = F) %>% unique()
intra$class <- "intra"
...
write.table(df_ATAC, file = "./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/ATAC_seq_datafiles/Raw_datatables/ZHR_Z30_ATAC_counts_ALLclasses_centered1000.txt", row.names = F, quote = F, sep = "\t")
...
write.table(df_ATAC, file = "./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/ATAC_seq_datafiles/Raw_datatables/ZHR_Z30_ATAC_counts_ALLclasses_20min_centered1000.txt", row.names = F, quote = F, sep = "\t")
...
write.table(df_ATAC_CPM, file = "./Integrative_AS_genomics/AS_ATAC_RNA_2020_10_1/ATAC_seq_datafiles/CPM_transformed_datatables/ZHR_Z30_ATAC_counts_ALLclasses_20min_CPM_centered1000.txt", row.names = F, quote = F, sep = "\t")
